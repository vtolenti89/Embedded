...
WiFiEspClient wifi;
WebSocketClient client = WebSocketClient(wifi, url, httpPort);
...
void loop()
{
  ...
  client.begin("/socket.io/?EIO=3&transport=websocket");
...
void getReq (char * url, char * endpoint) {
  ...
  if (wifi.connect(url, httpPort)) {
    Serial.println("Connecting...");
    // Make a HTTP request
    wifi.println("GET " + String(endpoint) +" HTTP/1.1");
    wifi.println("Host: " + String(url));
